...
def _pillow2array(img, flag='color', channel_order='bgr'):
    ...
    if flag == 'unchanged':
        ...
        if flag == 'color':
            ...
            raise ValueError(
                'flag must be "color", "grayscale" or "unchanged", but got %s'%flag)
    return array
...
def imread(img_or_path, flag='color', channel_order='bgr', backend=None):
    ...
    if backend not in supported_backends:
        raise ValueError('backend: %s is not supported. Supported '
                         "backends are 'cv2', 'turbojpeg', 'pillow'"%backend)
    ...
    if isinstance(img_or_path, np.ndarray):
        ...
        check_file_exist(img_or_path, 'img file does not exist: %s'%img_or_path)
