...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None, style='pytorch', with_cp=False):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=conv2_stride,
...
def make_res_layer(block, inplanes, planes, blocks, stride=1, dilation=1, style='pytorch', with_cp=False):
    ...
    layers.append(block(inplanes, planes, stride, dilation,
                        downsample, style=style, with_cp=with_cp))
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_stages=4, strides=(1, 2, 2, 2), dilations=(1, 1, 1, 1), out_indices=(0, 1, 2, 3), style='pytorch', frozen_stages=(- 1), bn_eval=True, bn_frozen=False, with_cp=False):
        ...
        if (depth not in self.arch_settings):
            raise KeyError(
                ''.join(['invalid depth ', '{}'.format(depth), ' for resnet']))
        ...
        for (i, num_blocks) in enumerate(stage_blocks):
            ...
            planes = (64 * (2 ** i))
            res_layer = make_res_layer(block, self.inplanes, planes, num_blocks,
                                       stride=stride, dilation=dilation, style=self.style, with_cp=with_cp)
            self.inplanes = (planes * block.expansion)
            layer_name = ''.join(['layer', '{}'.format((i + 1))])
            self.add_module(layer_name, res_layer)
            self.res_layers.append(layer_name)
        self.feat_dim = ((block.expansion * 64) *
                         (2 ** (len(stage_blocks) - 1)))
    ...
    def train(self, mode=True):
        ...
        if (mode and (self.frozen_stages >= 0)):
            ...
            for i in range(1, (self.frozen_stages + 1)):
                mod = getattr(self, ''.join(['layer', '{}'.format(i)]))
                mod.eval()
