...
class SyncBatchNormFunction(Function):
    ...
    def forward(self, input, running_mean, running_var, weight, bias, momentum,
        ...
        assert isinstance(
               ...
               'only support Half or Float Tensor, but {input.type()}'
...
class SyncBatchNorm(Module):
    ...
    def forward(self, input):
        if input.dim() < 2:
            raise ValueError(
                'expected at least 2D input, got {input.dim()}D input')
    ...
    def __repr__(self):
        ...
        s += 'affine={self.affine}, '
        s += 'track_running_stats={self.track_running_stats}, '
