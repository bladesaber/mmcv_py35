...
class MomentumUpdaterHook(Hook):
    ...
    def __init__(self, by_epoch=True, warmup=None, warmup_iters=0, warmup_ratio=0.9):
        ...
        if (warmup is not None):
            ...
            assert (0 < warmup_ratio <=
                    1.0), '"warmup_momentum" must be in range (0,1]'
    ...
    def get_warmup_momentum(self, cur_iters):
        if (self.warmup == 'constant'):
            warmup_momentum = [(_momentum / self.warmup_ratio)
                               for _momentum in self.regular_momentum]
        elif (self.warmup == 'linear'):
            k = ((1 - (cur_iters / self.warmup_iters)) * (1 - self.warmup_ratio))
            warmup_momentum = [(_momentum / (1 - k))
                               for _momentum in self.regular_mom]
        elif (self.warmup == 'exp'):
            k = (self.warmup_ratio ** (1 - (cur_iters / self.warmup_iters)))
            warmup_momentum = [(_momentum / k)
                               for _momentum in self.regular_mom]
    ...
    def before_run(self, runner):
        ...
        self.base_momentum = [group['initial_momentum']
                              for group in runner.optimizer.param_groups]
...
class CyclicMomentumUpdaterHook(MomentumUpdaterHook):
    ...
    def before_run(self, runner):
        ...
        self.momentum_phases.append([iter_up_phase, max_iter_per_phase,
