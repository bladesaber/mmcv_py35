    ...
    def __init__(self, by_epoch=True, warmup=None, warmup_iters=0, warmup_ratio=0.1, warmup_by_epoch=False):
        if (warmup is not None):
            if (warmup not in ['constant', 'linear', 'exp']):
                raise ValueError(''.join(['"', '{}'.format(
                    warmup), '" is not a supported type for warming up, valid types are "constant" and "linear"']))
        if (warmup is not None):
            assert (warmup_iters > 0), '"warmup_iters" must be a positive integer'
            assert (0 < warmup_ratio <=
                    1.0), '"warmup_ratio" must be in range (0,1]'
    ...
    def get_regular_lr(self, runner):
        if isinstance(runner.optimizer, dict):
            ...
            for k in runner.optimizer.keys():
                _lr_group = [self.get_lr(runner, _base_lr)
...
class CosineRestartLrUpdaterHook(LrUpdaterHook):
    ...
    def get_lr(self, runner, base_lr):
        ...
        nearest_restart = (
            0 if (idx == 0) else self.cumulative_periods[(idx - 1)])
...
def get_position_from_periods(iteration, cumulative_periods):
    ...
    for (i, period) in enumerate(cumulative_periods):
        if (iteration < period):
            return i
    raise ValueError(''.join(['Current iteration ', '{}'.format(
        iteration), ' exceeds cumulative_periods ', '{}'.format(cumulative_periods)]))
...
class CyclicLrUpdaterHook(LrUpdaterHook):
    ...
    def __init__(self, by_epoch=False, target_ratio=(10, 0.0001), cyclic_times=1, step_ratio_up=0.4, **kwargs):
        if isinstance(target_ratio, float):
            target_ratio = (target_ratio, (target_ratio / 100000.0))
        elif isinstance(target_ratio, tuple):
            target_ratio = ((target_ratio[0], (target_ratio[0] / 100000.0))
                            if (len(target_ratio) == 1) else target_ratio)
        else:
            raise ValueError(''.join(
                ['target_ratio should be either float or tuple, got ', '{}'.format(type(target_ratio))]))
        assert (len(target_ratio) ==
                2), '"target_ratio" must be list or tuple of two floats'
        assert (0 <= step_ratio_up <
                1.0), '"step_ratio_up" must be in range [0,1)'
    ...
    def get_lr(self, runner, base_lr):
        ...
        for (start_iter, end_iter, max_iter_per_phase, start_ratio, end_ratio) in self.lr_phases:
            ...
            if (start_iter <= curr_iter < end_iter):
                ...
                return annealing_cos((base_lr * start_ratio), (base_lr * end_ratio), (progress / (end_iter - start_iter)))
