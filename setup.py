...
try:
    ...
    if (int(major) < 3):
        raise RuntimeError(''.join(
            ['OpenCV >=3 is required but ', '{}'.format(cv2.__version__), ' is installed']))
except ImportError:
    CHOOSE_INSTALL_REQUIRES = [
        ('opencv-python-headless>=3', 'opencv-python>=3')]
...
setup(name=('mmcv' if (os.getenv('MMCV_WITH_OPS', '0') == '0') else 'mmcv-full'), version=get_version(), description='OpenMMLab Computer Vision Foundation', keywords='computer vision', packages=find_packages(), include_package_data=True, classifiers=['Development Status :: 4 - Beta', 'License :: OSI Approved :: Apache Software License', 'Operating System :: OS Independent', 'Programming Language :: Python :: 3',
                                                                                                                                                                                                                                                           'Programming Language :: Python :: 3.6', 'Programming Language :: Python :: 3.7', 'Programming Language :: Python :: 3.8', 'Topic :: Utilities'], url='https://github.com/open-mmlab/mmcv', author='MMCV Authors', author_email='openmmlab@gmail.com', setup_requires=['pytest-runner'], tests_require=['pytest'], install_requires=install_requires, ext_modules=get_extensions(), cmdclass=cmd_class, zip_safe=False)
